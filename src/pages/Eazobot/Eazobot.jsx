...
const Eazobot = () => {
  ...
  const platforms = [
    ...
    },
    {
      ...
      description: 'An advanced bot builder for developers powered by Zoho Creator Deluge Scripts.',
      icon: 'ƒx',
      selected: false
    },
    // {
  ...
  const generateScript = () => {
    return `<script>window.$zoho=window.$zoho || {};$zoho.salesiq=$zoho.salesiq||{ready:function(){}}</script><script id="zsiqscript" src="https://salesiq.zoho.com/widget?wc=siq${Math.random().toString(36).substr(2, 9)}" defer></script>`;
  };
  ...
  const renderStepIndicator = () => (
    <div className="flex items-center justify-center mb-8 bg-white border-b border-gray-200 py-4">
      <button
        onClick={prevStep}
        ...
        {steps.map((step, index) => (
          <React.Fragment key={step.id}>
            <div className="flex items-center">
              ...
              <span className={`ml-2 text-sm font-medium ${step.id === currentStep ? 'text-blue-600' : 'text-gray-600'
                }`}>
                {step.title}
              </span>
            </div>
  ...
  const renderBotBuilder = () => (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      <div className="mb-8">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">Choose a platform</h3>
        ...
        <p className="text-gray-600 mb-6">
          Build a compelling chatbot with our in-house bot-building platforms to suit your business needs.
        </p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {platforms.map((platform) => (
            <div
              key={platform.id}
              className={`border-2 rounded-lg p-6 cursor-pointer transition-colors ${platform.selected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'
                }`}
  ...
  const renderBotProfile = () => (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <div className="bg-white rounded-lg">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Customize chat widget</h2>

          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Pick a color</h3>
            <div className="flex space-x-3">
              {colors.map((color) => (
                <button
                  key={color}
                  onClick={() => setConfig({ ...config, color })}
                  className={`w-12 h-12 rounded-full border-2 flex items-center justify-center ${config.color === color ? 'border-gray-400' : 'border-transparent'
                    }`}
                  style={{ backgroundColor: color }}
                >
                  {config.color === color && <FiCheck className="text-white" size={20} />}
                </button>
              ))}
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-4 w-80">
              <div className="flex justify-between items-center mb-3">
                <div className="flex items-center">
                  ...
                  ></div>
                  <span className="text-sm font-medium text-gray-700">{config.greetingMessage}</span>
                </div>
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      <div className="flex">
        ...
        <div className="flex-1">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-2">Choose bot audience</h2>
            <p className="text-gray-600 mb-6">Select the type of visitor to initiate proactive conversation.</p>

            <div className="grid md:grid-cols-2 gap-6 mb-8">
              <div
                className={`border-2 rounded-lg p-6 cursor-pointer ${config.audience === 'all' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}
                onClick={() => setConfig({ ...config, audience: 'all' })}
              >
                <h3 className="font-semibold text-gray-900 mb-2">All visitors</h3>
              ...
              <div
                className={`border-2 rounded-lg p-6 cursor-pointer ${config.audience === 'custom' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}
                onClick={() => setConfig({ ...config, audience: 'custom' })}
              >
                <h3 className="font-semibold text-gray-900 mb-2">Custom visitors</h3>
                <p className="text-gray-600 text-sm mb-4">
                  The bot will assist visitors if they match the set condition and criteria.
                </p>
                <button className="text-blue-500 text-sm font-medium">Add rule</button>
              </div>
            </div>

            <div className="mb-8">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">When should the bot initiate chat?</h3>
              <div className="space-y-3">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setConfig({ ...config, initiateTrigger: [...config.initiateTrigger, 'land'] });
                      } else {
                        setConfig({ ...config, initiateTrigger: config.initiateTrigger.filter(t => t !== 'land') });
                      }
                    }}
                    className="mr-3 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-gray-700">When visitors land on the site</span>
                ...
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setConfig({ ...config, initiateTrigger: [...config.initiateTrigger, 'click'] });
                      } else {
                        setConfig({ ...config, initiateTrigger: config.initiateTrigger.filter(t => t !== 'click') });
                      }
                    }}
                    className="mr-3 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-gray-700">When visitors click the chat widget</span>
                  <FiInfo className="ml-2 text-gray-400" size={16} />
                </label>
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setConfig({ ...config, initiateTrigger: [...config.initiateTrigger, 'action'] });
                      } else {
                        setConfig({ ...config, initiateTrigger: config.initiateTrigger.filter(t => t !== 'action') });
                      }
                    }}
                    className="mr-3 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-gray-700">When visitors perform the custom action</span>
                  <FiInfo className="ml-2 text-gray-400" size={16} />
                </label>
              </div>
            </div>
            ...
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Response interval</h3>
              <p className="text-gray-600 text-sm mb-4">
                Add an interval to your bot's response time during interactive chats for a more natural conversational flow.
              </p>
              <select
                value={config.responseInterval}
                onChange={(e) => setConfig({ ...config, responseInterval: e.target.value })}
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Idle chat handling</h3>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={config.idleChatHandling}
                    onChange={(e) => setConfig({ ...config, idleChatHandling: e.target.checked })}
    ...
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="mb-8">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Add live chat to your website</h2>
          <span className="ml-auto text-sm text-gray-500">Less than 1 min</span>
        </div>

        <p className="text-gray-600 mb-8">
          Your customized SalesIQ chat is all set and ready to go! Add it to your website now for your visitors to easily connect with you.
        </p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
          <div className="flex items-start mb-4">
            ...
            <div className="flex-1">
              <p className="text-gray-700 mb-4">
                Copy and paste the below code before the closing &lt;/body&gt; tag of your website's HTML source code.
              </p>
              ...
              <div className="mt-4 text-sm text-gray-600">
                ...
                <a href="#" className="text-blue-500 hover:text-blue-600 ml-1">📱 iOS</a>
                <span className="mx-1">☁️</span>
                <a href="#" className="text-blue-500 hover:text-blue-600">Android</a>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              <p className="text-gray-700 mb-4">
                Once the above code is added and published, the SalesIQ live chat will be up and running instantly on your website at the bottom right corner.
              </p>
              <p className="text-gray-600 text-sm">
                Don't see your widget? Check out our{' '}
                <a href="#" className="text-blue-500 hover:text-blue-600">Trouble shooting guide</a>
                {' '}or{' '}
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mb-6">Here are installation tutorials for various platforms</p>
