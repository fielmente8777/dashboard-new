...
const Usermanagement = () => {
    ...
    const deleteUserFromManagement = async (emailId) => {
        ...
        if (confirmation.isConfirmed) {
            try {
                const response = await fetch(`https://nexon.eazotel.com/user/delete/${emailId}`, {
                    method: "POST",
                    headers: {
    ...
    return (
        <>
            <div className='flex justify-end items-center'>
                <button onClick={() => setIsPopupOpen(true)} className='border-b-2 border-transparent hover:border-b-2 hover:border-[#575757] transition-all duration-300 px-4 py-3 bg-white font-medium text-[#575757]'>Add New user</button>
            </div>
                    ...
                    <table className="w-full text-left bg-white ">
                        <thead >
                            ...
                            <tr className="border-b whitespace-nowrap">
                                ...
                                <th className="py-2 text-[14px] font-medium text-[#575757] capitalize whitespace-nowrap">Action</th>
                        ...
                        <tbody className="">
                            {userManagementData?.map((user, index) => (
                                <tr key={index} className="border-b border-gray-200 hover:bg-[#f8f8fb] transition duration-300 cursor-pointer">
                                    ...
                                    <td>
                                        ...
                                        {user.isAdmin ? "" : <span className='flex items-center gap-3'> <MdDeleteOutline size={20} onClick={() => deleteUserFromManagement(user.emailId)} className='text-red-500 mt-[2px]' /> <FaEdit onClick={() => { handleEditUserData(user) }} className='text-black' /></span>}
                                    </td>
                                </tr>
