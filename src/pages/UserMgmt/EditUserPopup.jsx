...
import { UpdateUser } from "../../services/api/userManagement.api";
import { accessScopeMap, accessRoles } from "./UserMgmtPopup";
...
const EditUserPopup = ({
  ...
  useEffect(() => {
    ...
    if (Array?.isArray(editData?.assigned_location))
      setSelectedLocations([...editData?.assigned_location]);
  }, [editData]);
  ...
  const toggleRole = (locationHid, role) => {
    const roleKey = accessScopeMap[role];
    setSelectedLocations((prev) =>
      prev.map((entry) =>
        entry.hid === locationHid
          ? {
              ...entry,
              accessScope: {
                ...entry.accessScope,
                [roleKey]: !entry.accessScope[roleKey],
              },
            }
          : entry
      )
  ...
  const removeLocation = (locationHid) => {
    setSelectedLocations((prev) =>
      prev.filter((entry) => entry.hid !== locationHid)
    );
  };
    ...
    >
      <div className="bg-white grid grid-cols-8 gap-6 rounded-2xl shadow-xl p-8 w-full md:max-w-6xl max-h-[90vh] overflow-y-auto space-y-8 relative">
        ...
        <div className="col-span-5 space-y-8">
          ...
          {/* {selectedLocations.map((locEntry)a => (
            <div
              key={locEntry.location}
              className="mt-4 border border-primary/30 p-4 rounded-md relative shadow-md shadow-black/10 space-y-2"
            >
