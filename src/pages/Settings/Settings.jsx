...
const Settings = () => {
  ...
  return (
    <div className="">
        ...
        {tabs.map((item)=>(
            <span onClick={()=>setActiveTab(item)} className={`${activeTab===item?"bg-gray-200":"bg-white"} hover:bg-gray-200 px-5 transition-all duration-150 text-sm font-medium py-2 text-center  `}>{item}</span>
        ))}
        </div>
        ...
        {activeTab==="WhatsApp"&&
        <div>
            <WhatsAppBusiness/></div>}
        {activeTab==="Lead"&&<div></div>}
        {activeTab==="Google Ads"&&<div></div>}
        {activeTab==="Eazbot"&&<div>
             <Eazobot />
            </div>}
        {activeTab==="Integration"&&<div>
             <Integration />
            </div>}
