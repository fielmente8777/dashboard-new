...
const Analytics = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setWebsitedata(data);
            setfiltered(data?.Gallery.filter(item => item.Category === selectedCategory));
        } catch (error) {
    ...
    useEffect(() => {
        if (websitedata?.Gallery) {
            const filteredImages = websitedata.Gallery.filter(item => item.Category === selectedCategory);
            setfiltered(filteredImages);
        }
    }, [selectedCategory, websitedata]);
        ...
        <div className='bg-white p-4'>
            ...
            <div>
                <div className='flex gap-4 '>
                    {websitedata?.Gallery?.map((item, index) => (
                        <button onClick={() => setSelectedCategory(item.Category)} key={index} className={`text-[14px] ${selectedCategory === item.Category ? "border-b-2 border-[#575757]" : "border-b-2 border-transparent"} px-4 py-3 bg-white font-medium text-[#575757]`}>{item.Category}</button>
                    ))}
                </div>
                ...
                {loading ?
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-5">
                        {[1, 2, 3, 4].map((item, index) => (
                            <div key={index} className='h-[200px] animate-pulse bg-gray-100 rounded' />
                        ...
                        {filtered.map((item, idx) =>
                            item.Images.map((img, i) => (
                                <div key={i} className="relative h-[200px]">
                                    <img src={img} alt={item.Category} className="w-full h-auto rounded object-cover" />

                                    <MdDeleteOutline size={28} onClick={() => handleDeleteImage(item.Category, img, "remove")}
