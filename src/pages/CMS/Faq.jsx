...
const Analytics = () => {
    ...
    const deleteFaq = async (que, ans) => {
        ...
        if (confirmation.isConfirmed) {
            try {
                const response = await fetch(`https://nexon.eazotel.com/cms/operation/Faq`, {
                    method: "POST",
                    headers: {
    ...
    return (
        <div className='bg-white'>
            <div className='bg-white p-4'>
                <h2 className="text-sm font-semibold text-[#575757]">Frequently ask questions</h2>
            </div>
            <div className="px-4">

                {!loading ?
                    <>
                        {websitefaqdata.length > 0 ? (
                            websitefaqdata.map((faq, index) => (
                                <div key={index} className="border-b border-gray-200">
                                    ...
                                    >
                                        {faq.Question} {openIndex === index && <MdDeleteOutline size={20} onClick={() => deleteFaq(faq.Question, faq.Answer)} className='text-red-500 mt-[2px]' />}
                                    </button>
                                    {openIndex === index && (
                ...
                <div className='mt-4 rounded'>
                    ...
                    <div className='flex flex-col gap-4 mt-4'>
                        ...
                        <textarea
                            ...
                            rows="4"
                        />
                        <button onClick={addFaq} className="bg-[#0a3a75] text-white py-2 px-4 rounded">Add FAQ</button>
                    </div>
                </div>
            </div>
        </div>
