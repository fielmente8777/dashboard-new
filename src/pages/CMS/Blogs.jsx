...
const Blogs = () => {
    ...
    return (
        ...
        <div className='bg-white  p-4'>
            ...
            {loading ?
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-4'>
                    {[1, 2, 3, 4, 5, 6, 7, 8].map((item, index) => (
                        <div key={index} className="bg-gray-100 animate-pulse rounded-md p-4">
                            <h3 className="text-lg font-semibold text-[#575757]">Loading...</h3>
                            <p className="text-sm text-gray-600 mt-2">Loading...</p>
                            <a href="#" className="text-blue-500 hover:underline mt-2 block">Loading...</a>
                        </div>
                    ))}
                </div>
                :
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-4'>
                    {websiteBlogsData?.map((blog, index) => (
                        <div key={index} className="bg-gray-100  rounded-md p-4">
                            <h3 className="text-lg font-semibold text-[#575757]">{blog.Heading}</h3>
                            <p className="text-sm text-gray-600 mt-2">{blog.Text}</p>
                            <a href={blog?.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline mt-2 block">Read More</a>
                        </div>
                    ))}
                </div>
            }
            ...
            <div className='mt-5'>
                <h2 className="text-sm font-semibold capitalize text-[#575757]">Add new blog</h2>
            </div>

            <hr className='mt-2' />
            <div className='mt-2'>
                <label htmlFor='heading' className='text-sm font-semibold text-[#575757] mt-4'>Heading</label>
                <input
                    type="text"
                    id='heading'
            ...
            <div className='mt-2'>
                <label htmlFor='slug' className='text-sm font-semibold text-[#575757] mt-4'>Slug</label>
                <input
                    type="text"
                    id='slug'
                    className='w-full border outline-none text-[#575757]/70  border-gray-300 rounded p-2 mt-2'
            ...
            <div className='mt-4'>
                <div className='flex justify-between items-center'>
                    ...
                    {selectedImage && <button onClick={() => setSelectedImage("")} className='py-1 px-5 text-sm text-white bg-red-700 font-semibold'>Clear</button>}
                </div>

                <div className='h-[40dvh]'>
                    {selectedImage ?
                        <img src={selectedImage} alt="Preview" className=" border w-full object-cover object-top h-full rounded" />
                        :
                        <FileUploader handleChange={handleChange} name="selectedImage" id="file" className=
                            "h-full w-full"
            ...
            <div className='mt-4'>
                <label htmlFor='text' className='text-sm font-semibold text-[#575757] mt-4'>Text</label>
                <JoditEditor
                    ref={editor}
                    value={blogText}
                    onChange={(newContent) => {
            ...
            <div>
                <button onClick={addBlog} className='py-2 px-5 text-sm text-white bg-[#575757] font-semibold mt-4'>Add Blog</button>
            </div>
