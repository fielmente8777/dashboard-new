    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setWebsitedata(data);
                setwebsiteCancellationdata(data?.TermsConditions[1]?.Cancellation || "");
                setwebsiteTermsdata(data?.TermsConditions[2]?.TermsServices || "")
                setwebsitePrivacydata(data?.TermsConditions[0]?.Privacy || "")
            } catch (error) {
    ...
    return (
        <div className='bg-white p-4'>
            <div className='flex  justify-between items-center '>
                <h2 className="text-sm font-semibold text-[#575757]">Cancellation and Refund Policy</h2>
                <button className='text-sm font-semibold  bg-[#0a3a75] hover:bg-[#0a3a75]/90 text-white px-3 py-1 flex items-center rounded-sm text-[14px]' onClick={() => { update(text) }}>Update</button>
            </div>
