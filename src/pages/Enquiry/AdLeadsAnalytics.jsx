...
const AdLeadsAnalytics = () => {
    ...
    useEffect(() => {
        if (Leads && Leads.length > 0) {
            ...
            // Extract date range
            const dates = processedData?.map(lead => new Date(lead.created_time)).filter(d => !isNaN(d));
            if (dates.length > 0) {
                dates.sort((a, b) => a - b);
                setDateRange({
                    start: dates[0],
    ...
    const generateInsights = (data, headers) => {
        ...
        data.forEach(lead => {
            ...
            const guestCount = lead['how_many_guests_are_you_booking_for?'] || 'Unknown';
            insights.guestCounts[guestCount] = (insights.guestCounts[guestCount] || 0) + 1;
        ...
        insights.conversionRate = (insights.qualifiedLeads / insights.totalLeads * 100).toFixed(1);
        ...
        // Find duplicates
        const duplicateEmails = Object.entries(emails).filter(([_, count]) => count > 1);
        const duplicatePhones = Object.entries(phones).filter(([_, count]) => count > 1);
    ...
    const chartData = useMemo(() => {
        ...
        return {
            // Lead sources data
            leadSources: Object.entries(insights.leadSources).map(([name, value]) => ({
                name,
                value
            })),
            ...
            // Guest counts data
            guestCounts: Object.entries(insights.guestCounts).map(([name, value]) => ({
                name,
                value
            })),

            // Budget ranges data
            budgetRanges: Object.entries(insights.budgetRanges).map(([name, value]) => ({
                name,
                value
            })),
    ...
    const COLORS = ['#8884d8', '#82ca9d', '#ffc658', '#ff7300', '#00ff00', '#0088fe', '#ff0088'];
            ...
            <div className=" mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
                        <div className="mb-4 md:mb-0">
                            <h1 className="text-2xl md:text-lg font-semibold text-gray-900">Lead Analytics Dashboard</h1>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl shadow-sm p-5 border border-blue-100 transition-all hover:shadow-md hover:scale-[1.02]">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-3xl font-bold text-gray-800">{insights.totalLeads || 0}</p>
                            </div>
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl shadow-sm p-5 border border-green-100 transition-all hover:shadow-md hover:scale-[1.02]">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium text-green-600 mb-1">Qualified Leads</h3>
                                <p className="text-3xl font-bold text-gray-800">{insights.qualifiedLeads || 0}</p>
                            </div>
                        ...
                        <div className="mt-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${insights.qualifiedLeads > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                                }`}>
                                {insights.qualifiedLeads > 0 ? 'Good progress' : 'No qualifications yet'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl shadow-sm p-5 border border-purple-100 transition-all hover:shadow-md hover:scale-[1.02]">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium text-purple-600 mb-1">Conversion Rate</h3>
                                <p className="text-3xl font-bold text-gray-800">{insights.conversionRate || 0}%</p>
                            </div>
                        ...
                        <div className="mt-2 w-full bg-gray-200 rounded-full h-1.5">
                            <div
                                ...
                                style={{ width: `${Math.min(100, insights.conversionRate || 0)}%` }}
                            ></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl shadow-sm p-5 border border-amber-100 transition-all hover:shadow-md hover:scale-[1.02]">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium text-amber-600 mb-1">Top Location</h3>
                                <p className="text-3xl font-bold text-gray-800">
                                    {chartData.locations && chartData.locations[0]?.name || 'N/A'}
                                </p>
                            </div>
                        ...
                        {chartData.locations && chartData.locations[0]?.value && (
                            <p className="text-xs text-amber-600 mt-1">
                                {chartData.locations[0].value} leads ({Math.round((chartData.locations[0].value / insights.totalLeads) * 100)}%)
                            </p>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm mb-6">
                    <div className="border-b border-gray-200">
                        <nav className="flex overflow-x-auto">
                            {['overview', 'sources', 'geographics', 'budget', 'timeline'].map(tab => (
                                <button
                                    key={tab}
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div className="bg-white p-4 rounded-lg border border-gray-200">
                                    ...
                                    <div className="h-96">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <AreaChart
                                                data={Object.entries(chartData.leadsOverTime || {})?.map(([date, count]) => ({
                                                    date,
                                                    count
