...
const ConversationalTool = () => {
  ...
  return (
    <div className="bg-white p-4 flex gap-10">
          ...
          <ul>
            {chatWhat.map((item, index) => (
              <li
                ...
                className="border cursor-pointer py-2 px-2 bg-gray-50"
              >
                {item.name}
              </li>
            ))}
          ...
          <ul>
            {chatInsta.map((item, index) => (
              <li
                onClick={() => setActiveUser(index)}
                key={index}
                className="border cursor-pointer bg-gray-50 py-2 px-2"
              >
                {item.name}
              </li>
            ))}
          ...
          <ul>
            {chatFace.map((item, index) => (
              <li
                onClick={() => setActiveUser(index)}
                key={index}
                className="border bg-gray-50 cursor-pointer py-2 px-2"
              >
                {item.name}
              </li>
            ))}
      ...
      {active === "Whatsapp" && (
        <div className="border w-full max-w-md mx-auto flex flex-col h-[75dvh] rounded-2xl shadow-md overflow-hidden bg-white">
          {/* Chat Header */}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-2 bg-gray-50">
            {messages.map((msg, idx) => (
              <div
                key={idx}
                className={`max-w-[70%] w-fit px-4 py-2 rounded-xl text-sm ${
                  msg.from === "me"
