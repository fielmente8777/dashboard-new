...
const Contacts = () => {
  ...
  return (
    <div className="p-2">
      ...
      <button className="bg-primary py-2 px-5 text-white font-semibold" onClick={handleAddPopup}>Add new Contact</button>
          ...
          {currentItems.length > 0 ? (
            currentItems.map((row,index) => (
              ...
              >
                ...
                <td className=" flex-1 py-3 px-2">{row.added_from?.toLowerCase()==="eazobot"?"Eazbot":row.added_from}</td>
                ...
                <td >
                  <button 
                  onClick={(e)=>{e.stopPropagation(); handleDeleteContact(row._id)}} 
      ...
      <div><AddContactPopup open={open} setOpen={setOpen} isEdit={isEdit} contact={selectedContact} setSelectedContact={setSelectedContact} getContacts={getContacts} /></div>
    </div>
  );
};
