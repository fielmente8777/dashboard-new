  ...
  return (
    <>
      <button
        ...
        className={`flex items-center gap-2.5 rounded-xl border border-white/10 bg-gradient-to-r from-slate-900/95 via-indigo-950/95 to-violet-950/95 px-3.5 py-1 shadow-lg backdrop-blur-md ring-1 transition hover:ring-indigo-400/40 hover:scale-[1.02] cursor-pointer ${ringCls}`}
        title="View credits & usage"
      >
        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gradient-to-br from-amber-300 to-amber-500 shadow-inner">
          <Coins className="h-4 w-4 text-amber-950" strokeWidth={2.2} />
        </div>
        <div className="min-w-0 pr-1 text-left flex items-center gap-5">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-white leading-none">
            Credits
          </p>
        ...
        {!isLow && !isEmpty && (
          <span className="rounded-full bg-amber-500/50 px-2 py-0.5 text-[10px] font-semibold text-amber-100">
            Low
          </span>
        )}
      </button>
