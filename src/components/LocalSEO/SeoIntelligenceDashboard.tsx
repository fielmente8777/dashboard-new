...
> = {
  TARGET: {
    label: "Target",
    cls: "bg-emerald-400/10 text-emerald-300 ring-emerald-400/25",
    Icon: Target,
  },
  OPTIMIZE: {
    label: "Optimize",
    cls: "bg-amber-400/10 text-amber-300 ring-amber-400/25",
    Icon: Lightbulb,
  },
  DEFEND: {
    label: "Defend",
    cls: "bg-teal-400/10 text-teal-300 ring-teal-400/25",
    Icon: Crown,
  },
  RESEARCH: {
    label: "Research",
    cls: "bg-zinc-400/10 text-zinc-300 ring-zinc-400/20",
    Icon: Sparkles,
  },
};
...
const rankTone = (rank: number | null | undefined) => {
  if (rank == null)
    return {
      text: "text-zinc-500",
      dot: "bg-zinc-600",
      cell: "bg-zinc-800 text-zinc-500",
      label: "Not ranked",
    };
  if (rank <= 3)
    return {
      ...
      dot: "bg-emerald-400",
      cell: "bg-emerald-500 text-emerald-950",
      label: "Top 3",
    };
  if (rank <= 7)
    return {
      text: "text-teal-300",
      dot: "bg-teal-400",
      cell: "bg-teal-500 text-teal-950",
      label: "Top 7",
    };
  if (rank <= 15)
    return {
      text: "text-amber-300",
      dot: "bg-amber-400",
      cell: "bg-amber-500 text-amber-950",
      label: "Page 1–2",
    };
  return {
    text: "text-rose-300",
    dot: "bg-rose-400",
    cell: "bg-rose-500/80 text-rose-50",
    label: "Low",
  };
};
...
const ScanProgressToast = ({
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 w-80 rounded-2xl border ${borderColor} bg-zinc-900/95 p-4 shadow-2xl backdrop-blur-sm`}
    ...
    >
      ...
      {!isQueued && (
        <div className="mb-3 h-1.5 w-full overflow-hidden rounded-full bg-zinc-800">
          <div
            className={`h-full rounded-full transition-all duration-700 ${isDone ? "bg-emerald-500" : isFailed ? "bg-rose-500" : "bg-teal-500"}`}
      ...
      {progress.detail && (
        <p className="mt-1 text-[11px] text-zinc-500">{progress.detail}</p>
      )}
      {isDone && (
        <button
          onClick={() => {
      ...
      {isFailed && (
        <p className="mt-2 text-[11px] text-zinc-500">
          Will retry automatically on the next run.
        </p>
      )}
      {!isDone && !isFailed && (
...
const GeoGridScanningOverlay = ({
  ...
  >
    ...
    `}</style>
    <div
      style={{ position: "relative", width: 64, height: 64, marginBottom: 28 }}
    >
      <div
        style={{
          position: "absolute",
          ...
          animation: "gg-ring-outer 1.1s linear infinite",
        }}
      />
      <div
        style={{
          ...
          borderLeftColor: "rgba(0,212,170,0.15)",
          animation: "gg-ring-inner 0.8s linear infinite",
        }}
      />
      <div
        style={{
    ...
    <p
      style={{
        ...
        maxWidth: 240,
        lineHeight: 1.5,
        animation: "gg-fadein 0.6s ease",
      }}
    >
      Scanning geo-grid across all 25 points.
      <br />
      ...
      {[0, 1, 2].map((i) => (
        <div
          ...
          style={{
            ...
            borderRadius: "50%",
            background: "#00d4aa",
            animation: `gg-dots 1.4s ease-in-out ${i * 0.16}s infinite`,
          }}
        />
      ))}
    </div>
...
const GeoGrid = ({
  ...
  const isScanning =
    gridStatus === "scanning" || (gridStatus === "pending" && watchingGridScan);
    ...
    return (
      <div className="relative overflow-hidden rounded-3xl border border-dashed border-zinc-700/70 bg-zinc-900/40 p-8 flex flex-col items-center justify-center text-center h-[480px]">
        ...
        <p className="text-sm font-semibold text-zinc-200">
          Geo-grid not scanned yet
        </p>
        <p className="mt-1.5 max-w-xs text-xs text-zinc-500">
          Click Re-scan above to run your first geo-grid scan.
        </p>
        {lastScanned && (
          <p className="mt-3 text-[11px] text-zinc-600">
            Last scan: {new Date(lastScanned).toLocaleDateString("en-IN")}
          </p>
        )}
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {isScanning && (
        <GeoGridScanningOverlay
          onRescan={onRescan}
          triggeringScan={watchingGridScan}
        />
      )}
      {lastScanned && !isScanning && (
        ...
        >
          ...
          Updated{" "}
          {new Date(lastScanned).toLocaleDateString("en-IN", {
            day: "numeric",
            month: "short",
          })}
        </div>
...
const OnboardingWizard = ({
  ...
  useEffect(() => {
    ...
    const defaultLng = form.lng ? Number(form.lng) : 77.209;
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () =>
        setLocationError(
          "Could not get location. Please click on the map or enter manually.",
        ),
    );
  };
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      ...
      const bt = form.businessType,
        ci = form.city.toLowerCase(),
        di = form.district.toLowerCase();
      setSuggestions([
        `${bt} in ${di}`,
        `best ${bt} in ${di}`,
        `${bt} near ${di}`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(
        `${NODE_BASE_URL}/seo/seo-intelligence/onboarding`,
        {
          ...
          primaryKeyword:
            [...selectedKws][0] ||
            form.primaryKeyword ||
            `${form.businessType} in ${form.district}`,
        },
        authConfig,
      );

      const allKws = [...selectedKws];
      if (form.primaryKeyword && !allKws.includes(form.primaryKeyword))
        allKws.push(form.primaryKeyword);
      if (allKws.length > 0) {
        await axios.post(
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {STEPS.map((label, i) => (
            <React.Fragment key={i}>
              <div className="flex items-center gap-2">
                <div
                  className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all ${step === i + 1 ? "bg-emerald-500 text-zinc-950" : step > i + 1 ? "bg-emerald-900 text-emerald-300" : "bg-zinc-800 text-zinc-500"}`}
                >
                  {step > i + 1 ? <CheckCircle className="h-4 w-4" /> : i + 1}
                </div>
                <span
                  className={`text-xs font-medium ${step === i + 1 ? "text-white" : "text-zinc-500"}`}
                >
                  {label}
                </span>
              </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-[11px] text-zinc-500">
                  Use the exact name as on Google Business Profile.
                </p>
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-zinc-300">
              ...
              <div>
                ...
                <input
                  value={form.selfDomain}
                  onChange={(e) => set("selfDomain", e.target.value)}
              ...
              <button
                onClick={() => setStep(2)}
                disabled={form.businessName.trim().length <= 2}
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1.5 text-[11px] text-zinc-500 flex items-center gap-1">
                  ...
                  Click the map — coordinates, city and district will auto-fill
                  below.
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button
                  onClick={() => setStep(1)}
                  className="flex-1 rounded-xl border border-zinc-700 px-5 py-3 text-sm font-medium text-zinc-300 transition hover:bg-zinc-800"
                >
                  Back
                </button>
                <button
                  ...
                  disabled={
                    !(form.lat && form.lng && form.city && form.district)
                  }
            ...
            <div className="space-y-5">
              ...
              {selectedKws.size > 0 && (
                <p className="text-xs text-emerald-400">
                  ✓ {selectedKws.size} keyword
                  {selectedKws.size !== 1 ? "s" : ""} selected
                </p>
              )}
...
const DifficultyMeter = ({ value }: { value: number | null }) => {
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="w-7 text-right text-xs font-semibold tabular-nums text-zinc-300">
        {value}
      </span>
    </div>
  );
...
const ActionBadge = ({ type }: { type: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11px] font-semibold ring-1 ${cfg.cls}`}
    >
...
const RankChip = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-lg bg-zinc-800/70 px-2 py-1 text-xs font-bold tabular-nums ring-1 ring-zinc-700/60 ${t.text}`}
    >
      <Icon className="h-3.5 w-3.5" />
...
const StatCard = ({ icon: Icon, label, value, sub, accent, glow }: any) => (
  <div className="group relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 backdrop-blur-sm transition-all hover:border-zinc-700">
    <div
      className={`absolute -right-8 -top-8 h-28 w-28 rounded-full ${glow} opacity-[0.12] blur-2xl transition-opacity group-hover:opacity-25`}
    />
    <div className="flex items-start justify-between">
      ...
      <span
        className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${accent} text-zinc-950 shadow-lg`}
      >
...
const buildInsights = (keywords: any[]) => {
  ...
  const mapsGap = keywords
    .filter(
      (k) =>
        (k.searchVolume ?? 0) >= 30 &&
        (k.liveGmbRank == null || k.liveGmbRank > 3),
    )
  ...
  if (mapsGap[0]) {
    ...
    out.push({
      ...
      body: (
        <>
          <Hl>"{g.keyword}"</Hl> gets about <Hl>{fmt(g.searchVolume)}</Hl>{" "}
          searches/month locally, but you're not in the 3-pack.
        </>
      ),
    });
  }
  ...
  const close = keywords
    .filter(
      (k) =>
        k.liveOrganicRank != null &&
        k.liveOrganicRank > 3 &&
        k.liveOrganicRank <= 10,
    )
  ...
  if (close[0]) {
    ...
    out.push({
      ...
      body: (
        <>
          You rank <Hl>{fmtRank(c.liveOrganicRank)}</Hl> organically for{" "}
          <Hl>"{c.keyword}"</Hl>. A few quality links could move you into the
          top spots.
        </>
      ),
    });
  ...
  if (defend[0]) {
    ...
    out.push({
      ...
      body: (
        <>
          You hold <Hl>{fmtRank(d.liveGmbRank)}</Hl> on Maps for{" "}
          <Hl>"{d.keyword}"</Hl>. Keep reviews flowing.
        </>
      ),
    });
  }
...
const SmartInsightCard = ({ insights }: any) => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/60 p-[1px] backdrop-blur-sm">
      <div
        className={`pointer-events-none absolute -inset-24 bg-gradient-to-r ${ins.glow} to-transparent opacity-50 blur-3xl`}
      />
...
const RankStat = ({ label, rank, kind }: any) => {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900/50 px-4 py-3">
      <span
        className={`inline-flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-800 ${t.text}`}
      >
...
const KeywordDetailPanel = ({ row }: { row: any }) => {
  ...
  const tip =
    row.actionBadge === "DEFEND"
      ? "Hold position: keep GBP fresh — new photos, posts and review replies weekly."
      : row.actionBadge === "OPTIMIZE"
        ? "Tighten on-page targeting and earn 2–3 local citations to climb into the pack."
        : row.actionBadge === "TARGET"
          ? "Strong opportunity: build a dedicated page + GBP service entry for this term."
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                ...
                <span className="text-emerald-400">
                  {localConfig?.district || "your area"},{" "}
                  {localConfig?.city || ""}
                </span>
              </p>
            </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <p className="text-[11px] text-zinc-500">
            One per line or comma-separated. {parsed.length} keyword(s)
            detected.
          </p>
          {parsed.length > 0 && (
            <p className="text-xs text-zinc-400">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-zinc-800 px-6 py-4">
          <button
            onClick={onClose}
            className="rounded-xl px-4 py-2 text-sm font-medium text-zinc-300 transition hover:bg-zinc-800"
          >
            Cancel
          </button>
          <button
            onClick={() => onAdd(parsed)}
            disabled={saving || !parsed.length || !canAfford}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl">
        <div className="flex items-center justify-between border-b border-zinc-800 px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                Track organic ranking for an exact URL
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-zinc-500 mt-1">
              One per line. {parsed.length} keyword(s) detected.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-zinc-800 px-6 py-4">
          ...
          <button
            onClick={() => onAdd(parsed, url)}
            disabled={saving || !parsed.length || !url.trim()}
...
const EmptyState = ({ onAdd }: any) => (
  <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-zinc-800 bg-zinc-900/30 py-20 text-center">
    ...
    <p className="mt-1.5 max-w-sm text-sm text-zinc-500">
      Add local searches this property should win.
    </p>
    <button
      onClick={onAdd}
...
const SeoIntelligenceDashboard = () => {
  ...
  const [tokenPricing, setTokenPricing] =
    useState<SeoTokenPricing>(DEFAULT_SEO_PRICING);
  ...
  useEffect(() => {
    ...
    const shouldPoll =
      status === "scanning" || (status === "pending" && watchingGridScan);
  ...
  const {
    progress: scanProgress,
    isActive: scanToastActive,
    dismiss: dismissScan,
  } = useSeoProgress(onScanDone);

  const { summary, trend, keywords, provider, geoGrid, gridMeta, localConfig } =
    data;
  const isGridActivelyScanning =
    gridMeta?.status === "scanning" ||
    (gridMeta?.status === "pending" && watchingGridScan);
  ...
  const local = useMemo(() => {
    const list = keywords || [];
    const inPack = list.filter(
      (k: any) => k.liveGmbRank != null && k.liveGmbRank <= 3,
    ).length;
    const mapsRanked = list.filter((k: any) => k.liveGmbRank != null);
    const avgMaps = mapsRanked.length
      ? +(
          mapsRanked.reduce((s: number, k: any) => s + k.liveGmbRank, 0) /
          mapsRanked.length
        ).toFixed(1)
      : null;
  ...
  const handleUntrack = async (keyword: string) => {
    try {
      await axios.delete(
        `${NODE_BASE_URL}/seo/seo-intelligence/${encodeURIComponent(keyword)}`,
        getAuthConfig(),
      );
      setData((d: any) => ({
        ...d,
        keywords: d.keywords.filter((k: any) => k.keyword !== keyword),
      }));
    } catch {
      console.error("Untrack failed");
    }
  };
  ...
  const handleTriggerGridScan = async () => {
    ...
    try {
      await axios.post(
        `${NODE_BASE_URL}/seo/seo-intelligence/trigger-grid-scan`,
        {},
        getAuthConfig(),
      );
      await fetchTokens();
      ...
      if (status === 402) {
        const { required, available } = err.response.data;
        setGridScanError(
          `Not enough tokens. Need ${required}, have ${available}.`,
        );
        await fetchTokens();
        notifySeoTokensChanged();
      } else {
      ...
      setData((prev: any) => ({
        ...prev,
        gridMeta: { ...(prev.gridMeta || {}), status: "failed" },
      }));
      setWatchingGridScan(false);
    } finally {
  ...
  const handleRadiusChange = async (r: number) => {
    try {
      await axios.post(
        `${NODE_BASE_URL}/seo/seo-intelligence/update-radius`,
        { radiusKm: r },
        getAuthConfig(),
      );
      setData((prev: any) => ({
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-950 to-black p-6 sm:p-8">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400">
                  <span className="text-emerald-400 font-semibold">
                    {localConfig.district}, {localConfig.city}
                  </span>
                </p>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    if (!newKw) return;
                    await axios.post(
                      `${NODE_BASE_URL}/seo/seo-intelligence/update-grid-keyword`,
                      { keyword: newKw },
                      getAuthConfig(),
                    );
                    setData((prev: any) => ({
      ...
      {data.staleKeywords > 0 && (
        <div className="relative mb-4 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-amber-200">
          Data may be slightly stale — use the refresh button on each keyword
          row to update rankings ({data.staleKeywords} keyword
          {data.staleKeywords !== 1 ? "s" : ""} outdated).
        </div>
      )}
      ...
      <div className="relative mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="flex items-center gap-5 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 backdrop-blur-sm z-10">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-medium uppercase tracking-wider text-zinc-500">
              {localConfig?.district
                ? `${localConfig.district} Visibility`
                : "Local Visibility"}
            </p>
            <p className="mt-1 text-sm leading-relaxed text-zinc-300">
              <Hl>{local.inPack}</Hl> of <Hl>{keywords.length || 0}</Hl> tracked
              terms in the Map 3-Pack.
            </p>
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-zinc-800/70 px-2.5 py-1 font-semibold text-emerald-300 ring-1 ring-zinc-700">
                <MapPin className="h-3.5 w-3.5" /> Avg Maps{" "}
                {fmtRank(local.avgMaps)}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-sm font-bold text-white">
                  Map Coverage — {localConfig?.district || "Your Area"}
                </h2>
                <p className="text-xs text-zinc-500">
                  {gridMeta?.keyword
                    ? `"${gridMeta.keyword}"`
                    : "Primary keyword"}{" "}
                  · {localConfig?.radiusKm || 5}km
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {gridMeta?.status === "done" && (
                <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-400 ring-1 ring-emerald-500/20">
                  <CheckCircle className="h-3.5 w-3.5" /> Updated
                </span>
              )}
              {(gridMeta?.status === "scanning" ||
                (gridMeta?.status === "pending" && watchingGridScan)) && (
                <span className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500/10 px-2.5 py-1 text-[11px] font-semibold text-amber-400 ring-1 ring-amber-500/20">
                  <Loader2 className="h-3.5 w-3.5 animate-spin" /> Scanning…
                </span>
              )}
              {gridMeta?.status === "failed" && (
                <span className="inline-flex items-center gap-1.5 rounded-lg bg-rose-500/10 px-2.5 py-1 text-[11px] font-semibold text-rose-400 ring-1 ring-rose-500/20">
                  <AlertCircle className="h-3.5 w-3.5" /> Failed
                </span>
              )}
              <button
                onClick={handleTriggerGridScan}
              ...
              >
                <RefreshCw
                  className={`h-3 w-3 ${triggeringScan ? "animate-spin" : ""}`}
                />
                {triggeringScan
                  ? "Starting..."
                  : `Re-scan (${tokenPricing.geoGridScan} tokens)`}
              </button>
            </div>
          </div>
          {gridScanError && (
      ...
      <div className="relative mt-4 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          value={summary ? fmt(summary.totalKeywords) : "—"}
          sub={`in ${localConfig?.city || "your area"}`}
        ...
        <StatCard
          icon={Target}
          label="Opportunities"
          value={summary ? fmt(summary.opportunities) : "—"}
          sub="gap terms to win"
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {localConfig && (
                <p className="text-xs text-zinc-500">
                  Volume = {localConfig.city} only · not India-wide
                </p>
              )}
            </div>
          </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[11px] uppercase tracking-wider text-zinc-600">
                    ...
                    <th className="pb-3 pl-6 text-left font-semibold">
                      Difficulty
                    </th>
                      ...
                      <React.Fragment key={key}>
                        ...
                        >
                          <td className="py-3.5 pl-2">
                            <ChevronDown
                              className={`h-4 w-4 text-zinc-600 transition-transform ${isOpen ? "rotate-180 text-emerald-300" : "group-hover:text-zinc-400"}`}
                            />
                          </td>
                          ...
                          <td className="py-3.5 text-right font-semibold tabular-nums text-zinc-200">
                            {fmt(row.searchVolume)}
                          </td>
                          <td className="py-3.5 text-right tabular-nums text-zinc-300">
                            {fmtCurrency(row.cpc)}
                          </td>
                          <td className="py-3.5 text-center">
                            <RankChip rank={row.liveGmbRank} kind="maps" />
                          </td>
                          <td className="py-3.5 pl-6">
                            <DifficultyMeter value={row.keywordDifficulty} />
                          </td>
                          <td className="py-3.5">
                            <ActionBadge type={row.actionBadge} />
                          </td>
                          <td className="py-3.5 pr-2 text-right">
                            <div className="flex items-center justify-end gap-1">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleRefreshKeyword(row.keyword);
                                }}
                              ...
                              >
                                <RefreshCw
                                  className={`h-4 w-4 ${refreshingKw === row.keyword ? "animate-spin" : ""}`}
                                />
                              </button>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleUntrack(row.keyword);
                                }}
